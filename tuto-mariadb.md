# Documentation technique : Installation et Configuration d'une VM (MariaDB) 💻
...
## 3\. Configuration des services (MariaDB)
...
Collez le contenu suivant (l'image a été changée de `mysql:8.0` à **`mariadb:10.6`** et le volume de `mysql_data` à **`mariadb_data`** pour plus de clarté) :

```yaml
services:
  db:
    image: mariadb:10.6 # CHANGEMENT: Utilisation de l'image MariaDB
    environment:
      MYSQL_ROOT_PASSWORD: ciel12000
      MYSQL_DATABASE: surveillanceEauCanal
    volumes:
      - mariadb_data:/var/lib/mysql # CHANGEMENT: Changement du nom du volume
    ports:
      - "3306:3306"
...
volumes:
  mariadb_data: # CHANGEMENT: Changement du nom du volume
```
...
Collez le contenu suivant (l'extension **`pdo_mysql`** fonctionne pour MariaDB et n'a pas besoin d'être changée, mais je la laisse pour la compatibilité) :

```dockerfile
FROM php:8.2-apache

# Installation des extensions MySQL/PDO (elles supportent aussi MariaDB)
...
Collez le code source (la connexion PDO avec le driver **`mysql:host=db`** est **compatible** avec MariaDB) :
...
Lancez la construction et le démarrage des conteneurs.
...
## 6\. Initialisation de la Base de Données (MariaDB)
